...
# Building a GoogleAdsClient requires credentials; stub them so these tests
# run without Application Default Credentials.
_credentials_patch = patch(
    "ads_mcp.utils._create_credentials", return_value=AnonymousCredentials()
)


def setUpModule():
    _credentials_patch.start()


def tearDownModule():
    _credentials_patch.stop()
...
class TestToCamelCase(unittest.TestCase):
    ...
    def test_simple_snake_case(self):
        self.assertEqual(_to_camel_case("campaign_budget"), "campaignBudget")
    ...
    def test_match_type(self):
        self.assertEqual(_to_camel_case("match_type"), "matchType")
...
class TestConvertKeysToCamel(unittest.TestCase):
    ...
    def test_list_values(self):
        ...
        self.assertEqual(result["finalUrls"], ["https://example.com"])
        self.assertEqual(
            result["items"],
...
class TestToSnakeCase(unittest.TestCase):
    ...
    def test_simple_camel(self):
        self.assertEqual(_to_snake_case("Campaign"), "campaign")
    ...
    def test_with_acronym(self):
        self.assertEqual(_to_snake_case("CampaignBudget"), "campaign_budget")
...
class TestGoogleAdsMutateValidation(unittest.TestCase):
    ...
    def test_update_without_resource_name_raises(self):
        ...
        self.assertIn("resource_name is required", str(ctx.exception))
    ...
    def test_create_without_attributes_raises(self):
        ...
        self.assertIn("attributes dict is required", str(ctx.exception))

    @patch("ads_mcp.utils.execute_mutate")
    def test_create_calls_execute_mutate(self, mock_mutate):
        mock_mutate.return_value = ["customers/123/campaigns/456"]
        ...
        self.assertEqual(call_kwargs["service_name"], "CampaignService")
        self.assertEqual(call_kwargs["mutate_method_name"], "mutate_campaigns")
    ...
    def test_create_label(self, mock_mutate):
        ...
        mock_mutate.return_value = ["customers/123/labels/789"]
        ...
        call_kwargs = mock_mutate.call_args[1]
        self.assertEqual(call_kwargs["service_name"], "LabelService")
        self.assertEqual(call_kwargs["mutate_method_name"], "mutate_labels")

    @patch("ads_mcp.utils.execute_mutate")
    def test_create_with_nested_attributes(self, mock_mutate):
        """Tests generic mutate with nested attributes (keyword)."""
        mock_mutate.return_value = ["customers/123/adGroupCriteria/456~789"]
