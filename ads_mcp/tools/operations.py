...
from ads_mcp.tools.write import WRITE_ANNOTATIONS, write_mcp
...
def upload_click_conversions(
    ...
    for conv in conversions:
        ...
        json_format.ParseDict(camel_conv, utils.to_pb(cc))
    ...
    if response.partial_failure_error:
        result["partial_failure_error"] = response.partial_failure_error.message
    return result
...
def upload_conversion_adjustments(
    ...
    """
    client = utils.get_googleads_client()
    service = utils.get_googleads_service("ConversionAdjustmentUploadService")

    adjustments = []
    for adj in conversion_adjustments:
        ca = client.get_type("ConversionAdjustment")
        camel_adj = _convert_keys_to_camel(adj)
        json_format.ParseDict(camel_adj, utils.to_pb(ca))
...
def create_offline_user_data_job(
    ...
    """
    client = utils.get_googleads_client()
    service = utils.get_googleads_service("OfflineUserDataJobService")

    job = client.get_type("OfflineUserDataJob")
    job.type_ = utils.resolve_enum("OfflineUserDataJobTypeEnum", job_type)

    if external_id is not None:
        job.external_id = external_id

    if customer_match_user_list_metadata:
        camel = _convert_keys_to_camel(customer_match_user_list_metadata)
        json_format.ParseDict(
            camel, utils.to_pb(job.customer_match_user_list_metadata)
        )

    if store_sales_metadata:
        camel = _convert_keys_to_camel(store_sales_metadata)
        json_format.ParseDict(camel, utils.to_pb(job.store_sales_metadata))
...
def add_offline_user_data_job_operations(
    ...
    for op_spec in operations:
        ...
        if "create" in op_spec:
            ...
            json_format.ParseDict(camel, utils.to_pb(op.create))
        elif "remove" in op_spec:
            camel = _convert_keys_to_camel(op_spec["remove"])
            json_format.ParseDict(camel, utils.to_pb(op.remove))
    ...
    for op_spec in operations:
        ...
        if action == "create":
            ...
            if attributes:
                ...
                json_format.ParseDict(camel, utils.to_pb(resource))
        elif action == "update":
        ...
        client.copy_from(getattr(mutate_op, op_field), entity_operation)
...
def upload_user_data(
    ...
    if customer_match_user_list_metadata:
        metadata = client.get_type("CustomerMatchUserListMetadata")
        camel = _convert_keys_to_camel(customer_match_user_list_metadata)
        json_format.ParseDict(camel, utils.to_pb(metadata))
...
write_mcp.add_tool(
    Tool.from_function(apply_recommendation, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(dismiss_recommendation, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(upload_click_conversions, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(upload_call_conversions, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(
        upload_conversion_adjustments, annotations=WRITE_ANNOTATIONS
    )
)
write_mcp.add_tool(
    Tool.from_function(
        create_offline_user_data_job, annotations=WRITE_ANNOTATIONS
    )
)
write_mcp.add_tool(
    Tool.from_function(
        add_offline_user_data_job_operations, annotations=WRITE_ANNOTATIONS
    )
)
write_mcp.add_tool(
    Tool.from_function(run_offline_user_data_job, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(add_batch_job_operations, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(run_batch_job, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(upload_user_data, annotations=WRITE_ANNOTATIONS)
)
