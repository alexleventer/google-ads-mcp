...
def update_campaign(
    ...
    """
    if all(v is None for v in [name, status, start_date_time, end_date_time]):
...
write_mcp.add_tool(
    Tool.from_function(create_campaign, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(update_campaign, annotations=WRITE_ANNOTATIONS)
)
write_mcp.add_tool(
    Tool.from_function(remove_campaign, annotations=WRITE_ANNOTATIONS)
)
